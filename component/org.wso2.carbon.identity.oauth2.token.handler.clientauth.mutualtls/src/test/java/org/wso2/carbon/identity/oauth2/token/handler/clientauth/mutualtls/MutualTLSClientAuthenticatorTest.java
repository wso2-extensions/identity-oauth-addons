...
public class MutualTLSClientAuthenticatorTest {
    ...
    private static String TEST_JSON_WITH_X5C = "{\n" +
            ...
            "    \"use\" : \"sig\",\n" +
            "    \"x5c\" : [ \"MIIDmzCCAoOgAwIBAgIJAJuzH6NrV5s5MA0GCSqGSIb3DQEBCwUAMGQxCzAJBgNVBAYTAlNMMQswCQYDVQQIDAJXUDEQMA4GA1UEBwwHQ29sb21ibzENMAsGA1UECgwE\n" +
            "d3NvMjENMAsGA1UECwwEd3NvMjEYMBYGA1UEAwwPdHJhdmVsb2NpdHkuY29tMB4X\n" +
            "DTE4MDIwNjEwNTk1N1oXDTE5MDIwNjEwNTk1N1owZDELMAkGA1UEBhMCU0wxCzAJ\n" +
            "BgNVBAgMAldQMRAwDgYDVQQHDAdDb2xvbWJvMQ0wCwYDVQQKDAR3c28yMQ0wCwYD\n" +
            "VQQLDAR3c28yMRgwFgYDVQQDDA90cmF2ZWxvY2l0eS5jb20wggEiMA0GCSqGSIb3\n" +
    ...
    private static MockedStatic<OAuth2Util> mockedOAuth2Util;
    private static MockedStatic<MutualTLSUtil> mockedMutualTLSUtil;
    private static MockedStatic<IdentityUtil> mockedIdentityUtil;
    private static MockedStatic<IdentityConfigParser> mockedConfigParser;

    @BeforeClass
    public void setUpSuite() throws Exception {
        // 1. Reset Singletons to clear previous "Fail" states in the JVM
        ...
        // 3. MOCK THE PARSER FIRST
        // This is the "Nuclear Option". By mocking the parser's getInstance, 
        // we stop it from ever running its constructor or buildConfiguration logic.
        mockedConfigParser = Mockito.mockStatic(IdentityConfigParser.class);
        IdentityConfigParser mockParserInstance = Mockito.mock(IdentityConfigParser.class);
        mockedConfigParser.when(IdentityConfigParser::getInstance).thenReturn(mockParserInstance);

        // 4. MOCK IDENTITY_UTIL
        mockedIdentityUtil = Mockito.mockStatic(IdentityUtil.class, Mockito.CALLS_REAL_METHODS);
        mockedIdentityUtil.when(IdentityUtil::getIdentityConfigDirPath).thenReturn(carbonHome);

        // 5. NOW INSTRUMENT THE TARGET UTILS
        // Since the Parser is now a harmless mock, OAuth2Util loading won't crash.
        mockedOAuth2Util = Mockito.mockStatic(OAuth2Util.class);
        mockedMutualTLSUtil = Mockito.mockStatic(MutualTLSUtil.class);
    }

    @AfterClass
    public void tearDownSuite() {

        if (mockedConfigParser != null) mockedConfigParser.close();
        if (mockedOAuth2Util != null) mockedOAuth2Util.close();
        if (mockedMutualTLSUtil != null) mockedMutualTLSUtil.close();
    ...
    public Object[][] testAuthenticateClientWhenJWKSEndPointGiven() {
        ...
        return new Object[][]{
                ...
                {getCertificate(CERTIFICATE_CONTENT), bodyParamsWithClientId, buildOAuthClientAuthnContext(CLIENT_ID),
                        true, TEST_JSON_WITH_X5T},
                {getCertificate(CERTIFICATE_CONTENT), bodyParamsWithClientId, buildOAuthClientAuthnContext(CLIENT_ID),
                        false, TEST_JSON},
                {getCertificate(CERTIFICATE_CONTENT), bodyParamsWithClientId, buildOAuthClientAuthnContext(CLIENT_ID),
                        true, TEST_JSON_X5T_XCT}
        };
    }
    ...
    public Object[][] testClientAuthnData() {
        ...
        return new Object[][]{
                {getCertificate(CERTIFICATE_CONTENT), new HashMap<String, List>(),
                        buildOAuthClientAuthnContext(CLIENT_ID), true},
                {getCertificate(CERTIFICATE_CONTENT), bodyParamsWithClientId, buildOAuthClientAuthnContext(CLIENT_ID),
                        true},
    ...
    public void testAuthenticateClient(Object certificate, HashMap<String, List> bodyContent,
        ...
        // FIX: Pre-calculate the certificate BEFORE starting the mockito 'when' block
        X509Certificate x509Certificate = getCertificate(CERTIFICATE_CONTENT);

        mockedMutualTLSUtil.when(() -> MutualTLSUtil.isJwksUriConfigured(any())).thenReturn(false);
        mockedOAuth2Util.when(() -> OAuth2Util.getTenantDomainOfOauthApp(ArgumentMatchers.anyString()))
                .thenReturn("carbon.super");

        // Use the pre-calculated variable here
        mockedOAuth2Util.when(
                        () -> OAuth2Util.getX509CertOfOAuthApp(oAuthClientAuthnContext.getClientId(), "carbon.super"))
        ...
        assertEquals(mutualTLSClientAuthenticator.authenticateClient(httpServletRequest, bodyContent,
                oAuthClientAuthnContext), authenticationResult);
    }
    ...
    public void testCanAuthenticateWithHeader(X509Certificate certificate,
        ...
        mockedIdentityUtil.when(() -> IdentityUtil.getProperty(CommonConstants.MTLS_AUTH_HEADER))
                .thenReturn("x-wso2-mtls-cert");
        Mockito.when(httpServletRequest.getHeader("x-wso2-mtls-cert")).thenReturn(CERTIFICATE_CONTENT3);
    ...
    public void testGetName() {
        ...
        assertEquals(mutualTLSClientAuthenticator.getName(), "MutualTLSClientAuthenticator",
                "Mutual TLS client authenticator name has changed.");
    }
    ...
    public void testAuthenticateClientWhenJWKSEndPointGiven(Object certificate, HashMap<String, List> bodyContent,
        ...
        MutualTLSClientAuthenticator mutualTLSClientAuthenticatorSpy = Mockito.spy(mutualTLSClientAuthenticator);
        OAuthClientAuthnContext oAuthClientAuthnContext = (OAuthClientAuthnContext) oAuthClientAuthnContextObj;
        HttpServletRequest httpServletRequest = Mockito.mock(HttpServletRequest.class);

        mockedMutualTLSUtil.when(() -> MutualTLSUtil.isJwksUriConfigured(any())).thenReturn(true);
        mockedOAuth2Util.when(() -> OAuth2Util.getTenantDomainOfOauthApp(ArgumentMatchers.anyString()))
                .thenReturn(SUPER_TENANT_DOMAIN_NAME);
        mockedOAuth2Util.when(
                        () -> OAuth2Util.getX509CertOfOAuthApp(oAuthClientAuthnContext.getClientId(), SUPER_TENANT_DOMAIN_NAME))
                .thenReturn(null);
        mockedMutualTLSUtil.when(() -> MutualTLSUtil.getThumbPrint(any(), any()))
                .thenReturn("da39a3ee5e6b4b0d3255bfef95601890afd80709");

        Mockito.doReturn(getJsonArray(testJson)).when(mutualTLSClientAuthenticatorSpy).getResourceContent(any());
        Mockito.doReturn(new URL("https://wso2is.com/.well-known/jwks.json"))
                .when(mutualTLSClientAuthenticatorSpy).getJWKSEndpointOfSP(any(), any());

        Mockito.when(httpServletRequest.getAttribute(JAVAX_SERVLET_REQUEST_CERTIFICATE)).thenReturn(certificate);

        assertEquals(mutualTLSClientAuthenticatorSpy.authenticateClient(httpServletRequest, bodyContent,
                        oAuthClientAuthnContext),
                authenticationResult, "Expected client authentication with JWKS expected result was not received");
    }
    ...
    public void testGetJWKSEndpointOfSP() throws Exception {
        ...
        mockedMutualTLSUtil.when(() -> MutualTLSUtil.getPropertyValue(any(), any()))
                .thenReturn("https://wso2is.com/.well-known/jwks.json");
    ...
    public void testAuthenticateWhenDifferentCertificates() throws Exception {
        ...
        // STUBBING: Tell the mock to return different strings for the two different certs
        mockedMutualTLSUtil.when(() -> MutualTLSUtil.getThumbPrint(cert, null))
                .thenReturn("THUMBPRINT_A");
        mockedMutualTLSUtil.when(() -> MutualTLSUtil.getThumbPrint(anotherCert, null))
                .thenReturn("THUMBPRINT_B");

        // Now the internal StringUtils.equals("THUMBPRINT_A", "THUMBPRINT_B") will return false
        assertFalse(mutualTLSClientAuthenticator.authenticate(cert, anotherCert),
                "Authentication should have failed for two different certificates.");
    }
    ...
    private X509Certificate getCertificate(String certificateContent) {
        ...
        if (StringUtils.isNotBlank(certificateContent)) {
            try {
                // IdentityUtil conversion logic is kept intact via CALLS_REAL_METHODS in setup
     ...
     * This bypasses 'Cannot instrument class' errors by clearing internal states
     * that crashed during previous static initialization attempts.
     */
    private void resetSingleton(Class<?> clazz) throws Exception {

        try {
            // Most WSO2 singletons store their instance in a private field named 'instance'
            Field instanceField = clazz.getDeclaredField("instance");
            instanceField.setAccessible(true);
            instanceField.set(null, null);
        } catch (NoSuchFieldException e) {
            // Fallback: Some versions of IdentityConfigParser use 'parser' instead of 'instance'
            try {
                Field parserField = clazz.getDeclaredField("parser");
                parserField.setAccessible(true);
                parserField.set(null, null);
            } catch (NoSuchFieldException ex) {
                // If neither field exists, the class might not be a standard singleton
            }
        }
    }
}