...
@WithCarbonHome
public class MTLSTokenBindingClientCredentialsGrantHandlerTest extends PowerMockTestCase {

    MTLSTokenBindingClientCredentialsGrantHandler mtlsTokenBindingClientCredentialsGrantHandler;
    ...
    public void testValidateScope() throws IdentityOAuth2Exception {
        ...
        mtlsTokenBindingClientCredentialsGrantHandler = new MTLSTokenBindingClientCredentialsGrantHandler();
        mockStatic(Oauth2ScopeUtils.class);
        PowerMockito.when(Oauth2ScopeUtils.validateByApplicationScopeValidator(Matchers.
                        any(OAuthTokenReqMessageContext.class),
                Matchers.any(OAuthAuthzReqMessageContext.class))).thenReturn(false);
        OAuthTokenReqMessageContext oAuthTokenReqMessageContext = new
                OAuthTokenReqMessageContext(oauth2AccessTokenReqDTOObject());
        oAuthTokenReqMessageContext.setScope(new String[]{"openid"});
        boolean validateScope =
                mtlsTokenBindingClientCredentialsGrantHandler.validateScope(oAuthTokenReqMessageContext);
